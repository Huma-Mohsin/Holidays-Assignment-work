...
function passwordValidation(password) {
    ...
    if (password.length < minLength || password.length > maxLength) {
        return chalk.yellowBright("Your password must be between 5 and 10 characters long.");
    }
    //Declare Variables
    ...
    if (!containsUpperCase ||
        ...
        !containsSpecialChar ||
        !containsNumber) {
        return chalk.redBright("Your password is weak. It should contain at least one uppercase letter, one lowercase letter, one special character, and one number.");
    }
    else {
